...
class Ebay
  ...
  def makeURL(type)
    ...
    # puts url
    charset = nil;
    contents = open(BaseURL) { |f| charset = f.charset; f.read }
    doc = Nokogiri::HTML(contents, nil, charset)
    main_category_links = doc.at('a:contains("Смартфоны и аксессуары")')['href']
    
    contents = open(category_links) { |f| charset = f.charset; f.read }
    doc = Nokogiri::HTML(contents, nil, charset)
    sub_category_links = doc.at('a:contains("Мониторы сотового телефона")')['href']
    
    contents = open(category_links) { |f| charset = f.charset; f.read }
    doc = Nokogiri::HTML(contents, nil, charset)
    all_products_page_links = doc.at('a:contains("Мониторы сотового телефона")')['href']
